...
def train_one_epoch(
    ...
    if os.getenv("KN_DYNAMIC_WEIGHT", False):  # TEST dynamic weight
        if cur_epoch < 5:
            cfg.model_cfg.w_bd_loss = [0.1, 0.1, 0.1, 0.1]
            cfg.model_cfg.w_stage_loss = 10
        elif cur_epoch < 15:
            cfg.model_cfg.w_bd_loss = [3, 0.01, 0.01, 0.01]
            cfg.model_cfg.w_stage_loss = 1
        elif cur_epoch < 15:
            cfg.model_cfg.w_bd_loss = [0.01, 3, 0.01, 0.01]
            cfg.model_cfg.w_stage_loss = 1
        elif cur_epoch < 20:
            cfg.model_cfg.w_bd_loss = [0.01, 0.01, 3, 0.01]
            cfg.model_cfg.w_stage_loss = 1
            cfg.model_cfg.topk = 5
        elif cur_epoch < 30:
            cfg.model_cfg.w_bd_loss = [0.01, 0.01, 0.01, 3]
            cfg.model_cfg.w_stage_loss = 1
        else:
            cfg.model_cfg.w_bd_loss = [0.5, 1, 2, 3]
            cfg.model_cfg.w_stage_loss = 3
            cfg.model_cfg.topk = 1
