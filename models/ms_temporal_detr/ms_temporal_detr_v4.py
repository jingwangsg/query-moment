...
from kn_util.nn_utils.layers import get_activation_fn
import os
...
class TransformerDecoderLayer(nn.Module):
    """
    original: decode query on the same memory
    decode query on different memory in pararllel
    """

    def __init__(self, d_model, nhead, ff_dim, dropout=0.1) -> None:
        super().__init__()
    ...
    def with_pos(self, x, pos):
        return x if pos is not None else x + pos

    def forward(self, tgt, memory, query_pos):
        B, Nq, _ = tgt.shape
        q = k = self.with_pos(tgt, query_pos)
        tgt = self.self_attn(query=q, key=k, value=tgt)[0]
        tgt = self.ln1(self.do(tgt) + tgt)

        q_flatten = repeat(q, "b nq d -> (b nq) i d", i=1)
        if len(memory.shape) == 3:
            memory = repeat(memory, "b n d -> (b nq) n d", nq=Nq)
        else:
            memory = rearrange(memory, "b nq n d -> (b nq) n d")
        x = self.cross_attn(query=q_flatten, key=memory, value=memory)[0]
        x = x.squeeze(1)
        x = rearrange(x, "(b nq) d -> b nq d", b=B)
        tgt = self.ln2(self.do(x) + tgt)

        x = self.w2(self.do(self.activation(self.w1(tgt))))
        tgt = self.ln3(self.do(x) + tgt)
...
class QueryBasedDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        layer = TransformerDecoderLayer(d_model, nhead, ff_dim, dropout=dropout)
        ...
        pool_ffn = MLP([d_model * num_scales, d_model, d_model])
    ...
    def forward(self, feat_lvls, reference_centers):
        ...
        for group_idx in range(GROUP_IDX):
            ...
            for idx, layer in enumerate(self.layers):
                output = layer(tgt, memory, query_pos=query_embeddings)  # B, Nq, D
                ...
                # TEST: align multi-scale feature by time
                pooled_feat = torch.cat(pooled_feat_list, dim=-1)
